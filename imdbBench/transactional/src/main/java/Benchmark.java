...
public class Benchmark extends Thread {
    ...
    private static Map<Utils.TransactionType, Long[]> rtPerType = new HashMap<>();
    ...
    public synchronized static void logTransaction(long tr, boolean success, Utils.TransactionType type) {
        if (started && !stopped) {
            n++;
            total += tr;
            if (!success) {
    ...
    public synchronized static void stopBench() {
        ...
        System.out.println("Response time per function (s)");
        System.out.println("------------------------------");
        rtPerType.forEach((k, v) -> System.out.println(k + " = " + v[0] / (v[1] * 1e9d)));
    ...
    public void run() {
        try {
            ...
            while(!isStopped()) {
                ...
                Utils.TransactionType type = null;
                try {
                    type =  wl.transaction();
